...
def test_sell_artifacts_live_invokes_signalr_with_single_argument(monkeypatch) -> None:
    fake_socket = FakeWebSocket(
        [
            "{}\x1e",
            '{"type":3,"invocationId":"1","result":true}\x1e',
            '{"type":1,"target":"HelperRequestResult","arguments":[{"request":"SellArtifacts","isSuccess":true,"error":""}]}\x1e',
        ]
    )
    ...
    assert result["status"] == "success"
    assert result["requested_count"] == 2
    assert result["helper_result"]["request"] == "SellArtifacts"
    assert '"target": "SellArtifacts"' in fake_socket.sent[1]
    assert '"arguments": [[100, 101]]' in fake_socket.sent[1]
